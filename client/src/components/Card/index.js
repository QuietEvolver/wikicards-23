...
export default class Card extends React.Component { //Card consists of deconstructed { props body }//{ title, abstract, confidence, uri, image, SaveButton }
  ...
  render() {
    const { id, title, abstract, confidence, uri, image, handleCardSave, firstTimeNewCardLoad } = this.props; //, handleInputChange, handleCardSave, q; bc we have not fxn, all attr are gotten from pty; will add components to Form to round out dyanmic formation.
    return (  //fxnCard returns the follwoing render from the page
      <ReactCardFlip isFlipped={this.state.isFlipped} infinite>
        <div key="front" className="card" onClick={this.handleCardClick.bind(this)} > {/*jsx transpiled by babel continued throughout .js pages rendering html<-->js; //imports the functionality of grand-child.div component of sibling List*/}
          ...
          </a> {/*firstTimeNewCardLoad = T then null; if both are T then 'hideBtn' happens */}
          { firstTimeNewCardLoad || this.state.hideButton ? null : <button 
            onClick={(e) => {
              handleCardSave(e)
              this.setState({ hideButton: true })//changes button to 'hide':T;
            }}
            className="btn"//button handler passing e props
          >
            New Card Save
          </button>
          }
